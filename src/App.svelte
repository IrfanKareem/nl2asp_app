...
<style>
  ...
  /* Summary cards row */
  .dash-summary { display: flex; gap: 0.65rem; margin-bottom: 1rem; flex-wrap: wrap; }
  .dash-model-card {
    flex: 1; min-width: 160px;
    background: #fff; border: 1.5px solid #e8ecf0; border-radius: 12px;
    padding: 0.85rem 1rem; box-shadow: 0 1px 4px rgba(0,0,0,0.04);
  }
  .dmc-header { display: flex; align-items: center; gap: 0.4rem; margin-bottom: 0.7rem; }
  .dmc-icon   { font-size: 1rem; }
  .dmc-label  { font-size: 0.75rem; font-weight: 700; color: #1e293b; flex: 1; min-width: 0;
    white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .dmc-tag    { font-family: 'JetBrains Mono', monospace; font-size: 0.54rem; color: #2563eb;
    background: #eff6ff; border: 1px solid #bfdbfe; padding: 1px 6px; border-radius: 4px; }
  .dmc-metrics { display: grid; grid-template-columns: 1fr 1fr; gap: 0.4rem; }
  .dmc-metric  { background: #f8fafc; border-radius: 8px; padding: 0.5rem 0.6rem; }
  .dmc-val     { font-size: 1.2rem; font-weight: 800; font-family: 'JetBrains Mono', monospace; line-height: 1; }
  .dmc-val.g   { color: #059669; } .dmc-val.b { color: #2563eb; }
  .dmc-lbl     { font-size: 0.55rem; color: #94a3b8; margin-top: 2px; text-transform: uppercase; letter-spacing: 0.05em; font-weight: 600; }
  .dmc-bar-wrap { margin-top: 0.5rem; }
  .dmc-bar-bg   { height: 4px; background: #f1f5f9; border-radius: 2px; overflow: hidden; }
  ...
  .cmp-table { width: 100%; border-collapse: collapse; font-size: 0.7rem; min-width: 700px; }
  .cmp-table th {
    text-align: left; padding: 0.5rem 0.75rem; background: #fafbfc;
    color: #94a3b8; font-family: 'JetBrains Mono', monospace; font-size: 0.57rem;
    text-transform: uppercase; letter-spacing: .07em; font-weight: 700;
    border-bottom: 1px solid #e8ecf0; white-space: nowrap;
  }
  .cmp-table th.model-header { background: #eff6ff; color: #2563eb; border-left: 2px solid #bfdbfe; }
  .cmp-table td {
    padding: 0.45rem 0.75rem; border-bottom: 1px solid #f8fafc;
    font-family: 'JetBrains Mono', monospace; color: #64748b;
    max-width: 180px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
    vertical-align: middle;
  }
  .cmp-table td.model-cell { border-left: 2px solid #f1f5f9; }
                ...
                <div class="dash-summary">
                  {#each dashStats as s}
                    <div class="dash-model-card">
                      ...
                      <div style="margin-top:0.5rem;font-size:0.6rem;color:#94a3b8;font-family:'JetBrains Mono',monospace">
                        {s.syntaxOk}/{s.total} valid · {s.compiled}/{s.total} compiled
                      </div>
                    </div>
                  {/each}
                </div>
                    ...
                    <thead>
                      <tr>
                        ...
                        {#each dashSelectedKeys as k}
                          <th class="model-header" colspan="2">{MODEL_INFO[k]?.icon} {MODEL_INFO[k]?.label ?? k}</th>
                        {/each}
                      </tr>
                      <tr>
                        <th colspan="7" style="height:0;padding:0;border:none"></th>
                        {#each dashSelectedKeys as k}
                    ...
                    <tbody>
                      {#each dashFilteredResults as r, i}
                        ...
                        <tr class:cmp-expanded={dashExpandedRow===idx}>
                          ...
                          <td title={r.nl} style="font-weight:500;color:#1e293b;max-width:180px">{r.nl}</td>
                        ...
                        {#if dashExpandedRow === idx}
                          <tr>
                            <td colspan={7 + dashSelectedKeys.length * 2} style="padding:0;border-bottom:1px solid #e2e8f0">
